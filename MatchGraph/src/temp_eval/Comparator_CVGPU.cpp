...
int ComparatorCVGPU::compareGPU(ImageHandler* iHandler, int* h_idx1,int* h_idx2, int* h_result, int k, bool showMatches)
{
	...
	for (int i=0; i < k && h_idx1[i] < iHandler->getTotalNr(); i++) {
		...
		try {
			...
			{
				h_result[i] = 1;
				std::cout << i1->path << "\t" << i2->path << std::endl;
			}
...
int main( int argc, char** argv )
{
	...
	comp.compareGPU(iHandler, a1, a2, res, sumK);
	//		for (int i=0; i<sumK; i++) {
	//			if (res[i] == 1) {
	//				printf("%s\t%s\n",
	//						iHandler->getFullImagePath(a1[i]),
	//						iHandler->getFullImagePath(a2[i]));
	//			}
	//	}
	delete a1;
	delete a2;
